...
class ArticleMaker < Mfweb::Core::Maker
  ...
  def draft?
    @is_draft || @should_show_all_installments
  end
  ...
  def show_all_installments
    @should_show_all_installments = true
  end

  def trim_future_installments
    @root.css('future-installment').each do |fi|
      future_elements = fi.children
        .reject{|e| 'installment-description' == e.name}
      future_elements.each do |e|
        if @should_show_all_installments        
          e.parent = fi.parent
        else
          e.remove
        end
